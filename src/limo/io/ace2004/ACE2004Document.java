...
public class ACE2004Document extends IAceRelationDocument {


	private SGMContent sgmContent;
	private ACE2004Reader ace2004Reader;
	

	public ACE2004Document(File xmlFile, ACE2004Reader ace2004reader) {
		this.xmlFile = xmlFile;
		this.ace2004Reader = ace2004reader;
		try {
	 ...
	 */
	private SGMContent _readSGMcontent(String url, String encoding) {
		return new SGMContent(url, encoding);
	}
	...
	public Entities readEntitiesAndMentionsFromFile() {
		if (this.initFileReader(this.xmlFile)) {
			...
			while (entityIter.hasNext()) {
				...
				//ignore entities not in mapping
				if (ace2004Reader.entityMapping != null) 
					if (ace2004Reader.entityMapping.containsKey(type))
						type = ace2004Reader.entityMapping.get(type);
					else 
						continue; //ignore entity
	...
	private void addMentions(Entity entity, Element entityElement) {
		...
		if (entity_mentions != null) {
			...
			while (mentionIter.hasNext()) {
				...
				//for now hard coded!
				/*boolean mapPREtype = true;
				if (mapPREtype) {
					if (mentionType.equals("PRE")) {
	...
	public Relations readRelationsFromFileAndAddToSentences() throws Exception {
		if (this.initFileReader(this.xmlFile)) {
	        ...
	        while (relationIter.hasNext()) {
	            ...
	            Iterator<Element> rel_entity_arg_iter = relElement.getChildren("rel_entity_arg").iterator();
				...
				if (ace2004Reader.keepRelations != null && !ace2004Reader.keepRelations.contains(type)) {
					System.err.println("Ignoring relation of type: "+type);
					continue;
				}
				...
				while (relIter.hasNext()) {
					...
					while (rel_mention_argument_iter.hasNext()) {
						...
						String entityMentionId =  relation_mention.getAttributeValue("ENTITYMENTIONID");
					...
					if (mention1.getHead().equals(mention2.getHead()))  {
						System.err.println("relation between mentions with same head! Ignore! mention1: "+mention1.getHead() + " mention2: "+mention2.getHead() 
								+ " Relation: "+uniqueRelationId);
						continue;
					}
					...
					if (this.ace2004Reader.relationsToIgnoreIds == null
							|| (this.ace2004Reader.relationsToIgnoreIds != null && !this.ace2004Reader.relationsToIgnoreIds
									.contains(relationId))) {
					...
					} else if  (this.ace2004Reader.relationsToIgnoreIds != null && this.ace2004Reader.relationsToIgnoreIds
							.contains(relationId)) {
	...
	public ArrayList<Sentence> readTextFromFileAndTokenize() {
		...
		for (List<HasWord> docProcSentence : docPreprocessor) {
			...
			for (HasWord iw : docProcSentence) {
				...
				//Token t = new Token(coreLab.value(), tokenCount, coreLab.beginPosition(), coreLab.endPosition()-1, coreLab.originalText(), sentence);
				Token t = new Token(coreLab.value().replace("\u00A0","_"), tokenCount, sentence);
				tokenCount += 1;
				...
				// set annotation indices from xmlfile
