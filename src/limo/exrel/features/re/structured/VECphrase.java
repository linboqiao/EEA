...
public class VECphrase extends RelationExtractionStructuredFeature {
	...
	protected String _extract(AbstractTree parseTree, Mention mention1,
		...
		try {
			...
			if (wordEmbeddingDictionary== null) {
				System.err.println("VECphrase: Did you load the Word Embedding file? Check config file.");
				System.exit(-1);
			}
			...
			String res = "";
			int start = wordEmbeddingDictionary.getDimension()*2 + 1;
			for (Double d : vec) {
	...
	private String zeroFeature(int dimension) {
		String res = "";
		for (int i = 2*dimension+1; i <= 3*dimension; i++) {
			res += i + ":0.0" + " ";
		}
		return res.trim();
	}
