...
//Fore-Between. Tokens before and between the two entities (unigram+bigrams+trigrams)
public class FB3 extends RelationExtractionLinearFeature {
	...
	protected String _extract(ParseTree parseTree, Mention mention1, Mention mention2,
		...
		for (int idx=0; idx < parts.length-2; idx++) {
			sb.append(parts[idx]+"_"+parts[idx+1]+"_"+parts[idx+2]);
			sb.append(RelationExtractionLinearFeature.BOWseparator);			
		}
		return sb.toString();
