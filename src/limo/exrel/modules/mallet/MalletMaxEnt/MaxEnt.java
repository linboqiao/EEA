...
//		assert (parameters.getNumCols() == defaultFeatureIndex+1);
	...
	public HashMap<String, String> getFeatureValues() {
		...
		for (int li = 0; li < numLabels; li++) {
			label = (String) labelDict.lookupObject(li);
			for (int i = 0; i < defaultFeatureIndex; i++) {
				name = (String) dict.lookupObject(i);
				weight = parameters[li*numFeatures + i];
				ret.put(name + " " + label, Integer.toString(i) + " " + Integer.toString(li) + " " + Double.toString(weight));
			}
		}
		return ret;
	}
