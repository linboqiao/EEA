     ...
     /* calculate the kernel function */
{
  ...
  switch(kernel_parm->kernel_type) {
    ...
    case 11: /* Re-ranking of predicate argument structures only trees*/
    ...
    case 14: /* Classification of Entailment pairs*/
            return((CFLOAT)ACL2005_Entailment_kernel(kernel_parm,a,b));
    case 15: /* Classification of Entailment pairs*/
            return((CFLOAT)Semantic_Tree_kernel(kernel_parm,a,b));  
...
//printf("Version file %s --- version label %s \n",version_buffer,VERSION);
  ...
  initializeSimilarities(model->kernel_parm.custom);
  ...
  for(i=1;i<model->sv_num;i++) {
    ...
    while(!isspace((int)line[++pos])); //go after alpha
		    ...
		    long int *totwords, long int *totdoc, KERNEL_PARM *kernel_parm)
{
  ...
  while((!feof(docfl)) && fgets(line,(int)ll,docfl)) {
    ...
    docs[dnum].instance_weight = doc.instance_weight;
		   ...
		   long int *totwords, long int max_words_doc, KERNEL_PARM *kernel_parm)
{
 ...
 //printf("\n\n---------------------------------------------------------\n\n");fflush(stdout);
  ...
  doc->instance_weight=1.0; //default instance weight is 1.0 (like was costfactor before)
  ...
  while(isspace((int)line[pos])) pos++;// go to "|BT|" marker

    read_tree_forest(line+pos, doc, &pos);// read the tree forest

  /* Look for Standard Features, pos returns the end of the last tree*/
  // printf("LINE:%s\n",line+pos);fflush(stdout);

    while(isspace((int)line[pos])&& line[pos]!=0) pos++;// go to "|BV|" marker
  /*while((!isspace((int)line[pos])) && line[pos]) pos++;*/
