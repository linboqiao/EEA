...
public class FlexibleOrthantWiseLimitedMemoryBFGS implements Optimizer {
	
	private static Logger logger = MalletLogger
			.getLogger(FlexibleOrthantWiseLimitedMemoryBFGS.class.getName());
	...
	//double l1Weight;
	RegularizationWeightProducer regularizer = new ConstantRegularizer(0.0);
	...
	int numFeatures = 0, numLabels = 0;

	public FlexibleOrthantWiseLimitedMemoryBFGS(Optimizable.ByGradientValue function, int numFeatures, int numLabels) {
		this(function, new ConstantRegularizer(0.0), numFeatures, numLabels);
	}

	public FlexibleOrthantWiseLimitedMemoryBFGS(Optimizable.ByGradientValue function,
			RegularizationWeightProducer regularizer, int numFeatures, int numLabels) {
		this.optimizable = function;
		//this.l1Weight = l1wt;
		this.regularizer = regularizer;
		if (regularizer == null)
			this.regularizer = new ConstantRegularizer(0.0);
		...
		assert(numParameters == (numLabels*numFeatures)):
			"Unmatched: numParameters = " + numParameters + " numLabels = " + numLabels + " numFeatures = " + numFeatures + " (" + (numLabels*numFeatures) + ")";
	...
	public boolean optimize(int numIterations) {
		logger.fine("Entering Flexible-OWL-BFGS.optimize(). L1 weight" 
				+ " Initial Value=" + value);
...
//		}
		...
		double param = 0, reg = 0;
		for (int li = 0; li < numLabels; li++)
			for (int fi = 0; fi < numFeatures; fi++) {
				param = parameters[li*numFeatures + fi];
				if (Double.isInfinite(param)) continue;
				reg = regularizer.getWeights(fi, li);
				if (reg > 0)
					sumAbsWt += Math.abs(param) * reg;
			}
		
		logger.info("getValue() (" + optName + ".getValue() = " + val
				+ " + pointwise |w|=" + sumAbsWt + ") = " + (val + sumAbsWt));
...
//	}
	private void makeSteepestDescDir() {
		...
		for (int li = 0; li < numLabels; li++) {
			for (int fi = 0; fi < numFeatures; fi++) {
				index = li*numFeatures + fi;
				reg = regularizer.getWeights(fi, li);
				
				if (parameters[index] < 0) {
					direction[index] = -grad[index] + reg;
				} else if (parameters[index] > 0) {
					direction[index] = -grad[index] - reg;
				} else {
					if (grad[index] < -reg) {
						direction[index] = -grad[index] - reg;
					} else if (grad[index] > reg) {
						direction[index] = -grad[index] + reg;
					} else {
						direction[index] = 0;
					}
...
//		}
		...
		for (int li = 0; li < numLabels; li++) {
			for (int fi = 0; fi < numFeatures; fi++) {
				...
				if (reg > 0 && (direction[index] * steepestDescentDirection[index]) <= 0)
					direction[index] = 0;
			}
		}
	}
		...
		for (int li = 0; li < numLabels; li++) {
			for (int fi = 0; fi < numFeatures; fi++) {
				...
				if (direction[index] != 0) {
					if (parameters[index] < 0) {
						val += direction[index] * (grad[index] - reg);
					} else if (parameters[index] > 0) {
						val += direction[index] * (grad[index] + reg);
...
//			}
			...
			for (int li = 0; li < numLabels; li++) {
				for (int fi = 0; fi < numFeatures; fi++) {
					...
					if (reg > 0 && (oldParameters[index] * parameters[index]) < 0)
