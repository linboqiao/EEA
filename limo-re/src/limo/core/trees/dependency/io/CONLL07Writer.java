 ...
 */
public class CONLL07Writer extends DependencyWriter {

    public CONLL07Writer (boolean labeled) {
	...
	for (int i=0; i<instance.length(); i++) {
		if (instance.forms[i].equals("<root>"))
			continue;
		...
		// skip i=0 since that is <root-feat>
		feat = feat + instance.feats[i+1][j];
		if (j < instance.feats[i+1].length-1)
		    feat = feat + "|";
	    }
	    if (feat == "") 
		feat = "-";*/
	    String feat = "-";
	    writer.write(feat);                                  writer.write('\t');
