...
def get_entity_vector(headwords, embeddings):
    entity_vec = np.zeros(embedding_len)
    for headword in headwords:
        try:
            entity_vec += embeddings[headword]
        except KeyError:
            pass
...
embeddings = gensim.models.Word2Vec.load_word2vec_format(
    '/home/hector/data/embeddings/mikolov/GoogleNews-vectors-negative300.bin', binary=True)
entities = {}
data_matrix = None
        ...
        for row in tsv:
            if len(row) > 1:
                ...
                if data_matrix is None:
                    data_matrix = get_entity_vector(headwords, embeddings)
                else:
                    data_matrix = np.vstack((data_matrix, get_entity_vector(headwords, embeddings)))
