 ...
 */
public class PlainJointTrainer extends AbstractLoggingAnnotator {
...
//    DDLatentTreeCrfDecoder decoder;
    ...
    private LatentTreeDecoder corefDecoder;

    private ViterbiDecoder mentionDecoder;
    ...
    private DummyCubicLagrangian dummayLagrangian = new DummyCubicLagrangian();
    ...
    public void process(JCas aJCas) throws AnalysisEngineProcessException {
        ...
        mentionDecoder.decode(mentionFeatureExtractor, mentionWeights, allMentions.size(), dummayLagrangian,
                dummayLagrangian, false);

        MentionGraph mentionGraph = new MentionGraph(allMentions.size(), mentionId2EventId, allMentionRelations);

        MentionSubGraph predictedTree = corefDecoder.decode(mentionGraph, corefWeights, corefFeatureExtractor,
                dummayLagrangian, dummayLagrangian);
        ...
        SequenceSolution prediction = mentionDecoder.getDecodedPrediction();
        double loss = goldSolution.loss(prediction);
        if (loss != 0) {
            GraphFeatureVector bestDecodingFeatures = mentionDecoder.getBestDecodingFeatures();
    ...
    private void prepareMentionTraining() throws ResourceInitializationException {
        ...
        mentionDecoder = new ViterbiDecoder(mentionFeatureAlphabet, mentionClassAlphabet);
    ...
    private void prepareCorefTraining() {
        ...
        corefDecoder = new BestFirstLatentTreeDecoder();
