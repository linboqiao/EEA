...
#define MAX_PARSE_TREE_LENGTH 50000
#define MAX_NUMBER_OF_NODES 10000
#define MAX_NUMBER_OF_PAIRS 100000
#define MAX_PRODUCTION_LENGTH 20000
#define MAX_NUMBER_OF_TREES 20000


#define MAX_NUMBER_OF_CONCEPTS	30000
...
//#include "../WordNet-1.7.1-compiled/include/wn.h"
//#include "../WordNet-1.7.1-compiled/include/wntypes.h" 

# define VERSION       "V5.01-TK-2.0" // TREE FOREST AND VECTOR SET
# define VERSION_DATE  "20.11.05"
...
# define CLASSIFICATIONS 1    /* train classification model */
...
typedef struct doc {
  ...
  double instance_weight; /* instance weight: if specified by -y */
...
typedef struct learn_parm {
  ...
  char predfile[400];          /* file for predicitions on unlabeled examples
				  in transduction */
  char alphafile[400];         /* file to store optimal alphas in. use  
...
typedef struct kernel_parm {
  ...
  double  coef_lin;
  double  coef_const;
  char    custom[400];    /* for user supplied kernel */ 
  double  tree_constant; /* Tree constant in t*K1 + K2 combined kernel --> B: use as alpha in: alpha*ets + (1-alpha)*ct */
...
extern double ACL2005_Entailment_kernel(KERNEL_PARM * kernel_parm, DOC * a, DOC * b);
extern double Semantic_Tree_kernel(KERNEL_PARM * kernel_parm, DOC * a, DOC * b); 

extern int getIndex(char* lemma);
extern float getSimilarity(int index1, int index2);
