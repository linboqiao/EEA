...
char docfile[400];           /* file with training examples */
char modelfile[400];         /* file for resulting classifier */

char weight_file[400];  /* file with instance weights */
...
{  
  ...
  //if(wninit()==-1)printf("\n\nERROR: Wordnet initialization\n\n");
  printf("reading: %s\n",kernel_parm.custom);
  initializeSimilarities(kernel_parm.custom);
  //readDictionary("dictionaries/dictionary_indices.txt");
  
 //printf("\n\n\n\n\n\n\nSimilarity %f\n\n", getSimilarity(1383,2325));
  ...
  printf("\nNumber of docs: %ld, linear space size: %ld\n\n",totdoc,totwords);
 
  if (strcmp(weight_file,"empty")!=0) {
    ...
    W = (double *)my_malloc(sizeof(double)*max_docs);
    int i,j;
    for(i=0;i<totdoc;i++){
      //printf("%d\n",i);
      fscanf(fp,"%lf",&W[i]);
      //printf("%lf\n",W[i]);
      docs[i].instance_weight = W[i];
 ...
 //if(kernel_parm.kernel_type==5) totwords=totdoc; // The number of features is proportional to the number of parse-trees, i.e. totdoc 
  ...
  if(kernel_parm.kernel_type == LINEAR) { /* don't need the cache */
    if(learn_parm.type == CLASSIFICATIONS) {
			   ...
			   LEARN_PARM *learn_parm,KERNEL_PARM *kernel_parm)
{
  ...
  for(i=1;(i<argc) && ((argv[i])[0] == '-');i++) {
    switch ((argv[i])[1]) 
      ...
      case 'y': i++; strcpy(weight_file,argv[i]); printf("Using weight file: %s\n",weight_file); break; /* instance weights */
  ...
  printf("-V %c\n",kernel_parm->vectorial_approach_standard_kernel);
  ...
  if(strcmp(type,"c")==0) {
    learn_parm->type=CLASSIFICATIONS;
  }
...
void print_help()
{
  ...
  printf("                        5: SubTree Kernel and SubSet Tree Kernel\n");
  printf("                        6: K1*K2 or r*K1 + K2 kernel combinations,\n");
  printf("                           where K1 = Tree kernel and K2 is a second kernel from 0 to 4\n");
  printf("                           defined above.\n");
  printf("         -D [0,1]    -> run the SubTree or the SubSet Tree kernel (default 1)\n");
  printf("         -L float    -> decay factor in tree kernel (default 0.4)\n");
  printf("         -C ['*','+']-> kernel combination operator between K1 and K2 (default '*')\n");
  printf("         -S [0,4]    -> second kernel K2 (default 1) \n");
  printf("         -T float    -> r value for K1 when '+' operator is used (default 0.3) \n");
  printf("         -N float    -> 0 = no normalization, 1 = K1 norm., 2 = K2 norm.,\n");
  printf("                        3 = K1 and K2 norm., default (= 3)\n");
