...
printf ("estimating ...\n");fflush(stdout);
  ...
  for(i=0;i<totdoc;i++) {    /* various inits */
    ...
    lin[i]=0;
    c[i]=0.0;
    //init c to instance weight
    //c[i]=docs[i].instance_weight;
    //printf("c[i]: %f\n",c[i]);
    //for instance weighting: use costfactor
    docs[i].costfactor = docs[i].instance_weight;
    //printf("docs[i].costfactor: %f\n",docs[i].costfactor);
...
void write_model(char *modelfile, MODEL *model)
{
  ...
  fprintf(modelfl,"%f # kernel parameter -D \n",model->kernel_parm.sigma);
