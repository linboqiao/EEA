...
void strToLower(char *concept){
char *pointer;
	  pointer=concept;

  	  while(*concept!=0){
	  *pointer=tolower(*concept);
...
/*char *lemmatize(char *concept, int POS){

char *lemma;

	strToLower(concept); // put in lower case 
	lemma=morphword(concept, POS); //lemmatise it as a noun
	if(lemma!=NULL)strcpy(concept,lemma); // if there is no lemma take the word 
...
// *** Read the Parse-Tree in the first line  
  ...
  while(V_start!=NULL && !EV){
        ...
        if(V_end == NULL){ 
             ...
             if(V_end == NULL)V_end=strstr(V_start,"|ET|");
             EV=1;
             if(V_end == NULL) {
...
void putDynamic(TreeNode *node){
	...
	if(node->iNoOfChildren>0){
	   ...
	   for(i=0;i<node->iNoOfChildren;i++){
	   	...
	   	putDynamic(temp[i]);
		if(temp[i]->iNoOfChildren==0){
		 //POS=1;
		//if(node->sName[0]=='N')POS=1; lemmatize according to the syntactic cat..not available in similarity now
		// if(node->sName[0]=='J')POS=2;
		// if(node->sName[0]=='V')POS=1;
		/* if(POS>0){
		   lemmatize(temp[i]->sName,POS);
		   index=getIndex(temp[i]->sName);
		   if(index>0){
		 	*(temp[i]->sName)=0;
		 	sprintf(temp[i]->sName,"000000%d",index);
		   }
		 }*/
		}
           }
...
*/				
	...
	//skip emtpy spaces in between brackets, i.e. ) (
	int len = strlen(input);
	char inputNoSpaces[len];
	int i=0;
	int j=0;
	for (i=0; i<len; i++) {
	  if (i>1 && i < (len-1)) {
...
void fillTheNodeList(TreeNode *N, OrderedTreeNode *list, int *counter){
    ...
    char production_real[MAX_PRODUCTION_LENGTH];


	*production=0;
	if(N->iNoOfChildren>0){
	   sprintf(production,"%s->",N->sName);
	   sprintf(production_real,"%s->",N->sName);
//	   printf("%s->",N->sName);
	   pre_term=1;
	   for(i=0;i<N->iNoOfChildren;i++){
	     ...
	     strcat(production_real," ");
	     strcat(production_real,N->pChild[i]->sName);
	     fillTheNodeList(N->pChild[i], list, counter);
	   }
	   list[*counter].node=N;   
	   list[*counter].sName=strdup(production);
	   N->production=strdup(production_real);
